	...
	$.extend({ tablesorterPager: new function() {
		...
		updatePageDisplay = function(table, p, completed) {
			var i, pg, s, $out, regex,
				...
				t = [],
				sz = p.size || page_size; // don't allow dividing by zero
			...
			if ( Math.min( p.totalPages, p.filteredPages ) >= 0 ) {
				...
				s = ( p.ajaxData && p.ajaxData.output ? p.ajaxData.output || p.output : p.output )
					...
					.replace(/\{page([\-+]\d+)?\}/gi, function(m,n){
						return p.totalPages ? p.page + (n ? parseInt(n, page_size) : 1) : 0;
					})
		...
		hideRowsSetup = function(table, p){
			p.size = parseInt( p.$size.val(), page_size ) || p.size;
			$.data(table, 'pagerLastSize', p.size);
			pagerArrows(p);
			if ( !p.removeRows ) {
				hideRows(table, p);
		...
		renderAjax = function(data, table, p, xhr, exception){
			...
			if ( typeof(p.ajaxProcessing) === "function" ) {
				...
				// the same parameters
				p.totalPages = Math.ceil( p.totalRows / ( p.size || page_size ) );
		...
		getAjaxUrl = function(table, p) {
			var c = table.config,
				...
				.replace(/\{page([\-+]\d+)?\}/, function(s,n){ return p.page + (n ? parseInt(n, page_size) : 0); })
		...
		enablePager = function(table, p, triggered){
			...
			p.size = $.data(table, 'pagerLastSize') || parseInt(p.$size.find('option[selected]').val(), page_size) || p.size || 10;
